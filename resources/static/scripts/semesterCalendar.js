jQuery.fn.extend({semesterCalendar:function(options,initCallback){
			...
			if(res.status==200){
				if(res.responseText!=""){
					...
					if(options.empty=="false" || options.empty==false  || options.empty==0){
							...
							e.val(result.semesters.y0[0].schoolYear+semesterI18n.academicYear+result.semesters.y0[0].name+semesterI18n.semester);
							e.next().find("#semesterCalendar_target").val(result.semesters.y0[0].id);
					}else{
						year.attr("index","-1");
						term.attr("index","-1");
						e.val(semesterI18n.allSemester);
					...
					if(result.yearIndex!="-1"){
						if(options.value==""){
							...
							e.val(semesterInYear[result.termIndex].schoolYear+semesterI18n.academicYear+semesterInYear[result.termIndex].name+semesterI18n.semester);
					...
					if(!(options.empty=="false" || options.empty==false  || options.empty==0)){
						...
						var yearTds = ["<td class='calendar-bar-td-blankBorder' onClick='jQuery(\"#allSemester\").click()'>"+semesterI18n.allSemester+"</td>"];
						var count = 0;
	...
	jQuery(this).focus(function(){
		...
		if(oldVal==""){
			...
			td.click();
			jQuery(this).val(semesterI18n.allSemester);
	...
	bar.find("#allSemester").click(function(){
		...
		bar.prev().val(semesterI18n.allSemester);
		...
		if(this.id!="allSemester"){
			...
			if((flag && input.attr("index")!=0) || (!flag && input.attr("index")!=tds.length-1)){
				if(isYear){
					...
					bar.prev().val(year.val()+semesterI18n.academicYear+term.val()+semesterI18n.semester);
				}
			}
		}
		...
		if(!jQuery(this).is(":empty")){
			...
			if(jQuery(this).attr("index")){				
				...
				for(var i=0;i<terms.length;i++){
					html+="<tr><td index='"+i+"' val='"+terms[i].id+"' class='"+(i==0?"ui-state-active":"calendar-bar-td-blankBorder")+"'>"+semesterI18n.singleSemester+"<span>"+terms[i].name+"</span></td></tr>";
				}
				...
				tb.nextUntil("table").next().find("td").unbind("click").click(function(){
					if(!jQuery(this).is(":empty")){
						...
						if(!jQuery(this).hasClass("allSemester")){
							...
							bar.prev().val(bar.find("#semesterCalendar_year").val()+semesterI18n.academicYear+term.val()+semesterI18n.semester);
				...
				bar.prev().val(input.val()+semesterI18n.academicYear+terms[0].name+semesterI18n.semester);
				termInput.focus();
			}
		}
		yearTb.parent().focus();
